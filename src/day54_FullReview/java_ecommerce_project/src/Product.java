...
public class Product {
    ...
    public String getCategoryName() throws Exception {
        // u have categoryid. please go to database and check all category ids
        // and find matching one and return it name corresponding fhat category id
        for(Category category: StaticConstants.CATEGORY_LIST){
            if(getCategoryId().toString().equals(category.getId().toString())){
                return category.getName();
            }
        }
        //return null; ==> bunu yazarsak ok fakat milk ürünü için çıktı null olacak bunu dev anlar ama kullanıcı anlamaz
